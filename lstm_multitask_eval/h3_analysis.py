...
def compute_critical_region_surprisal(gram_sent, ungram_sent):
    ...
    for i, ((gram_w, gram_score), (ungram_w, ungram_score)) in enumerate(zip(gram_sent, ungram_sent)):
        if gram_w != ungram_w:
            ...
            # Clean punctuation just in case it's attached to the word
            clean_g_word = gram_w.strip('.,!?;:').lower()
            clean_ug_word = ungram_w.strip('.,!?;:').lower()
...
def analyze_h3_reflexives(results):
    ...
    Group A: 'herself'
    Group B: 'himself' or 'themselves'
    """
    # Initialize tracking dictionaries for the two groups
    analysis = {
        'herself': {
            'deltas': [],
            'correct': 0,
    ...
    target_reflexives = {'herself', 'himself', 'themselves'}
    
    for test_name, test_data in results.items():
        # Optional: You can uncomment the next line to strictly filter by test name if your keys are clean
        # if 'reflexive' not in test_name.lower(): continue 
        ...
        for key, sentences in test_data.items():
            for i in range(0, len(sentences), 2):
                ...
                # Check if the grammatical word is one of our target reflexives
                if g_word in target_reflexives:
                    if g_word == 'herself':
                        group = 'herself'
                    else:
