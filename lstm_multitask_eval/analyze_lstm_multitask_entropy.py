...
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze H4: Entropy Reduction Magnitudes")
    parser.add_argument('--results_file', type=str, default='./multitask_lstm_entropy_results.pickle',
                        help='Path to LSTM entropy results pickle file')
    parser.add_argument('--output_dir', type=str, default='../h4_results/multitask',
...
def calculate_pair_er(gram_sent, ungram_sent, test_name):
    ...
    # Need n (the critical word) AND n+1 (to read H_after) to be valid
    if n < 0 or n + 1 >= len(gram_sent) or n + 1 >= len(ungram_sent):
        return None
        
    # H_before: entropy BEFORE the critical word (identical prefix → same for both)
    H_before = gram_sent[n][2]
    ...
    ER_gram   = max(H_before - H_after_gram,   0)
    ER_ungram = max(H_before - H_after_ungram, 0)

    # H1 cross-reference: surprisal at critical word
    # log_prob is stored as log P(word|context), so surprisal = -log_prob
    s_gram   = -gram_sent[n][1]
    s_ungram = -ungram_sent[n][1]
    delta_s  = s_ungram - s_gram  # positive = ungram more surprising (H1 correct)
...
def main():
    ...
    # Per-construction aggregation
    # Tracks both mean ER (original) and per-pair H4 accuracy (new)
    er_stats = defaultdict(lambda: {
        'sum_gram'  : 0.0,
        'sum_ungram': 0.0,
        'sum_delta_s': 0.0,
        'h4_correct': 0,    # pairs where ER_gram > ER_ungram
        'h1_correct': 0,    # pairs where delta_s > 0  [cross-reference]
        'count'     : 0,
    })
    ...
    lines = []
    lines.append("\n" + "=" * W)
    lines.append("HYPOTHESIS 4: ENTROPY REDUCTION AT CRITICAL REGION")
    lines.append("=" * W)
    lines.append(
    ...
    out_file = os.path.join(args.output_dir, 'h4_entropy_analysis.txt')
