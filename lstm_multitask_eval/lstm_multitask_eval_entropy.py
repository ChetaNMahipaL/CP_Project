...
def parse_args():
    parser = argparse.ArgumentParser(description="Test LSTM Entropy Reduction on grammatical constructions")
    
    parser.add_argument('--template_dir', type=str, default='../EMNLP2018/templates',
                        help='Location of the template files')
    parser.add_argument('--output_file', type=str, default='multitask_lstm_entropy_results.pickle',
                        help='File to store the entropy results')
    parser.add_argument('--model', type=str, default='../models/lstm_multi.pt',
                        help='Path to the trained model checkpoint')
    parser.add_argument('--lm_data', type=str, default='../models/lstm_multi.bin',
...
class LSTMEvaluator:
    def __init__(self, model_path, lm_data_path, device='cuda'):
        self.device = device
        logger.info(f"Loading LSTM model: {model_path}")
        ...
        try:
            logger.info(f"Loading Dictionary from {lm_data_path}")
            self.dictionary = torch.load(lm_data_path, map_location='cpu', pickle_module=dill)
            logger.info(f"Loaded Dictionary successfully")
            logger.info(f"Vocabulary size: {len(self.dictionary)}")
        except Exception as e:
            logger.error(f"Failed to load Dictionary: {e}")
            logger.error(f"Make sure the file contains a Dictionary object and dill is installed")
            raise
    ...
    def get_sentence_word_scores(self, sentence):
        """
        Score a sentence and return word-level scores and entropy.
        Returns list of (word, log_prob, entropy) tuples.
        """
        ...
        try:
            ...
            with torch.no_grad():
                ...
                for i in range(len(indices) - 1):
                    ...
                    token_entropy = entropies[i].item()
                    word = words[i + 1] if i + 1 < len(words) else '<UNK>'
                    word_scores.append((word, token_log_prob, token_entropy))
...
def evaluate_lstm(evaluator, all_test_sents):
    logger.info("Starting entropy evaluation...")
...
def main():
    ...
    evaluator = LSTMEvaluator(model_path=args.model, lm_data_path=args.lm_data, 
                             device=args.device)
    
    all_test_sents = load_test_sentences(args.template_dir, args.tests)
    results = evaluate_lstm(evaluator, all_test_sents)
