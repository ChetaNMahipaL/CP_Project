...
if torch.cuda.is_available():
    ...
    if args.cuda:
        print("ERROR: No CUDA GPUs available. Run without --cuda flag")
        sys.exit(1)

###############################################################################
# Load data
###############################################################################
sys.stderr.write('Starting data loading...\n')
...
sys.stderr.write('Creating corpus...\n')
...
sys.stderr.write('Corpus created successfully\n')
...
if args.test:
    ...
    sys.stderr.write('Batchifying training data...\n')
    ...
    sys.stderr.write('Data batchification complete\n')
...
if not args.test:
    ...
    sys.stderr.write(f'Vocab size: {ntokens}\n')
    ...
    if args.cuda:
        try:
            if (not args.single) and (torch.cuda.device_count() > 1):
                ...
                sys.stderr.write(f'Using {torch.cuda.device_count()} GPUs with DataParallel\n')
                model = nn.DataParallel(model,dim=1)
            try:
                device_id = torch.cuda.current_device()
                sys.stderr.write(f'Moving model to CUDA device {device_id}\n')
            except:
                sys.stderr.write(f'Moving model to CUDA (device auto-selected)\n')
            model.cuda()
            sys.stderr.write('Model moved to CUDA successfully\n')
        except RuntimeError as e:
            sys.stderr.write(f'ERROR: Failed to move model to CUDA: {e}\n')
            sys.stderr.write(f'CUDA available: {torch.cuda.is_available()}, Device count: {torch.cuda.device_count()}\n')
            sys.stderr.write(f'Please check CUDA installation or run without --cuda flag\n')
            raise
    else:
        sys.stderr.write('Model will run on CPU\n')

criterion = nn.CrossEntropyLoss()
sys.stderr.write('Criterion initialized - Ready to train\n')
...
def test_evaluate(test_lm_sentences, test_ccg_sentences, lm_data_source, ccg_data_source):
    ...
    if args.words:
        ...
        sys.stdout.write('\n')
    for i in tqdm(range(len(lm_data_source)+len(ccg_data_source)), desc='Testing'):
...
def evaluate(lm_data_source, ccg_data_source):
    ...
    total_steps = (lm_data_source.size(0) + ccg_data_source.size(0) - 1) // args.bptt
    for i in tqdm(range(0, lm_data_source.size(0) + ccg_data_source.size(0) - 1, args.bptt), total=total_steps, desc='Evaluating'):
        # TAG
...
def train():
    ...
    sys.stderr.write(f'Starting batch training with {len(order)} batches\n')
    for batch, i in tqdm(order, desc='Training', total=len(order)):#enumerate(range(0, train_lm_data.size(0) + train_ccg_data.size(0) - 1, args.bptt)):
        # TAG
...
if not args.test:
    sys.stderr.write(f'Starting training for {args.epochs} epochs\n')
    try:
        for epoch in range(1, args.epochs+1):
            sys.stderr.write(f'Starting epoch {epoch}\n')
            epoch_start_time = time.time()
            sys.stderr.write(f'Epoch {epoch}: Training...\n')
            train()
            sys.stderr.write(f'Epoch {epoch}: Evaluating...\n')
            ...
            print('-' * 89)
            sys.stderr.write(f'Epoch {epoch} complete\n')
            # Save the model if the validation loss is the best we've seen so far.
            if not best_val_loss or val_loss < best_val_loss:
                with open(args.save, 'wb') as f:
                    torch.save(model, f)
                    best_val_loss = val_loss
                sys.stderr.write(f'Model saved with validation loss {val_loss}\n')
            else:
                # Anneal the learning rate if no improvement has been seen in the validation dataset.
                lr /= 4.0
                sys.stderr.write(f'Learning rate annealed to {lr}\n')
    except KeyboardInterrupt:
        print('-' * 89)
        print('Exiting from training early')
        sys.stderr.write('Training interrupted by user\n')
else:
    sys.stderr.write('In test mode\n')
    # Load the best saved model.
    sys.stderr.write(f'Loading model from {args.save}\n')
    with open(args.save, 'rb') as f:
        model = torch.load(f, weights_only=False, pickle_module=dill)
    sys.stderr.write('Model loaded successfully\n')

    # Run on test data.
    sys.stderr.write('Running test evaluation\n')
    test_loss = test_evaluate(test_lm_sentences, test_ccg_sentences ,test_lm_data, test_ccg_data)
    print('=' * 89)
    print('| End of training | test loss {:5.2f} | test ppl {:8.2f}'.format(
        test_loss, math.exp(test_loss)))
    print('=' * 89)    
    sys.stderr.write('Test evaluation complete\n')