...
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze NPI surprisal for N-gram models (H2)")
    parser.add_argument('--results_file', type=str, required=True,
                        help='Path to the n-gram results pickle file')
    ...
    for test_name, test_data in npi_tests.items():
        ...
        for key, sentences in test_data.items():
            for i in range(0, len(sentences), 2):
                ...
                # STRUCTURAL INDEXING
                # We don't need strings. We know exactly where 'ever' is located.
                # simple_npi -> index 3
                # rc_npi -> index -4 (4th word from the end, assuming detached period)
                ever_idx = 3 if category == 'simple_npi' else -4
                
                try:
                    # item[1] is the log probability
                    gram_ever_logp = gram_sent[ever_idx][1]
                    ungram_ever_logp = ungram_sent[ever_idx][1]
                except IndexError:
                    # Failsafe in case a sentence is unexpectedly short
                    continue
                
                # Filter out -inf (common in n-grams for unseen transitions)
                if not np.isinf(gram_ever_logp) and not np.isinf(ungram_ever_logp):
                    # Surprisal = -log_prob
...
def main():
    ...
    print(f"Loading N-gram results from {args.results_file}...")
    results = load_results(args.results_file)
    npi_analysis = extract_npi_surprisal(results)
    
    print("\n" + "="*80)
    print(f"H2: N-GRAM NPI LICENSING SURPRISAL ANALYSIS ({os.path.basename(args.results_file)})")
    print("="*80)
