...
def parse_args():
    parser = argparse.ArgumentParser(description="Analyze GPT-2 results using Critical-Region Surprisal")
    
    parser.add_argument('--results_file', type=str, required=True,
                        help='Path to GPT-2 results pickle file')
    parser.add_argument('--output_dir', type=str, default='../h1_results/gpt2',
...
def compute_critical_region_surprisal(gram_sent, ungram_sent):
    ...
    S(w) = -log P(w)
    """
    # Find the critical region by comparing sentences token by token
    for i, ((gram_w, gram_score), (ungram_w, ungram_score)) in enumerate(zip(gram_sent, ungram_sent)):
        if gram_w != ungram_w:
            ...
            # Model correctly prefers the grammatical token if Delta S > 0
...
def print_overall_results(analysis):
    output = []
    output.append("\n" + "="*80)
    output.append("GPT-2 EVALUATION RESULTS - OVERALL (CRITICAL-REGION SURPRISAL)")
    output.append("="*80)
...
def print_condensed_results(analysis):
    output = []
    output.append("\n" + "="*80)
    output.append("GPT-2 EVALUATION RESULTS - CONDENSED (CRITICAL-REGION SURPRISAL)")
    output.append("="*80)
...
def print_full_results(analysis):
    output = []
    output.append("\n" + "="*90)
    output.append("GPT-2 EVALUATION RESULTS - FULL (CRITICAL-REGION SURPRISAL)")
    output.append("="*90)
