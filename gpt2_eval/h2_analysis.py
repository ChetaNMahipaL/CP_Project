    ...
    for test_name, test_data in npi_tests.items():
        ...
        for key, sentences in test_data.items():
            for i in range(0, len(sentences), 2):
                ...
                gram_ever_logp = next((score for word, score in gram_sent if 'ever' in word.lower()), None)
                ungram_ever_logp = next((score for word, score in ungram_sent if 'ever' in word.lower()), None)
