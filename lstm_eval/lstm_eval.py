...
def parse_args():
    parser = argparse.ArgumentParser(description="Test LSTM on grammatical constructions")
    
    parser.add_argument('--template_dir', type=str, default='../EMNLP2018/templates',
                        help='Location of the template files')
    parser.add_argument('--output_file', type=str, default='lstm_results.pickle',
...
class LSTMEvaluator:
    def __init__(self, model_path, lm_data_path, device='cuda'):
        ...
        # Load saved Dictionary object (pickled with dill)
        # Must use dill as pickle_module because Dictionary class is custom
        try:
