...
# Complete LSTM LM Evaluation Pipeline
# This script runs the full evaluation workflow end-to-end

set -e

echo "==================== LSTM Language Model Evaluation Pipeline ===================="
echo ""
...
if [ ! -f "$MODEL_FILE" ]; then
    ...
    echo "Please train the LSTM model first using:"
    echo "  cd ../word-language-model"
    echo "  python main.py --lm_data ../data/lm_data \\"
    echo "    --save ../models/lstm_lm.pt --save_lm_data ../models/lstm_lm.bin"
    exit 1
fi
...
# Step 1: Evaluate
echo "Step 1: Evaluating LSTM LM on syntactic constructions..."
...
python analyze_lstm_results.py \
    --results_file "$TEMPLATE_DIR/lstm_results.pickle" \
...
echo "  - ../results/lstm/analysis_overall.txt"
echo "  - ../results/lstm/analysis_condensed.txt"
echo "  - ../results/lstm/analysis_full.txt"
echo ""
echo "Next steps:"
echo "  1. View overall results: cat ../results/lstm/analysis_overall.txt"
echo "  2. View detailed results: cat ../results/lstm/analysis_full.txt"
...
echo "       --lstm_results ../EMNLP2018/templates/lstm_results.pickle"
echo ""
