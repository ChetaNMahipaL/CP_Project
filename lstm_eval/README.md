# LSTM Language Model Syntactic Evaluation

This directory contains code to evaluate the trained LSTM language model on syntactic constructions as described in "Targeted Syntactic Evaluation of Language Models" (Marvin & Linzen, EMNLP 2018).

## Overview

The evaluation tests whether the LSTM LM (trained on language modeling alone) can capture specific linguistic phenomena, including:
...
The LSTM model is trained on:
- **LM task**: Standard language modeling on general corpora (LM data only)

Unlike the multitask LSTM, this model does not have CCG supertagging training, so it learns linguistic patterns purely from language modeling signal.
...
- The trained LSTM model checkpoint: `models/lstm_lm.pt`
- The LM data dictionary file: `models/lstm_lm.bin`
...
The `dill` package is essential for loading the Dictionary object stored in `lstm_lm.bin`. It was used during model training to serialize the custom Dictionary class.

### 3. Model Files

Make sure you have the following trained model files:
- `../models/lstm_lm.pt` - The trained LSTM LM checkpoint
- `../models/lstm_lm.bin` - Dictionary for faster loading
...
### Step 2: Evaluate LSTM on Test Sentences
...
- `--model`: Path to the trained LSTM checkpoint (default: `../models/lstm_lm.pt`)
- `--lm_data`: Path to LM data file with dictionary (default: `../models/lstm_lm.bin`)
- `--output_file`: Where to save evaluation results (default: `lstm_results.pickle`)
...
**Output:** 
- `lstm_results.pickle` - Scores for all test sentences
...
- `condensed`: Grouped by construction type (e.g., all object relatives together) - `full`: Detailed results with sample sentences and token-level scores

**Output:** Analysis results saved to `../results/lstm/analysis_{mode}.txt`
...
To compare results with multitask LSTM and other models:
...
2. **Verify paths**: Check that `lstm_lm.bin` is in the correct location (`../models/lstm_lm.bin`)
3. **Recreate lstm_lm.bin**: If the file is corrupted, retrain the model:
   ```bash
   cd ../word-language-model
...
The `lstm_lm.bin` file contains a custom `Dictionary` class that requires:
...
1. Verify paths to `lstm_lm.pt` and `lstm_lm.bin` are correct
...
├── run_evaluation.sh                # Automated pipeline
├── simple_example.py                # Example workflow
...
```
INFO:__main__:Loading LSTM model: ../models/lstm_lm.pt
...
...
INFO:__main__:Results saved to ../EMNLP2018/templates/lstm_results.pickle
```
...
If you use this evaluation code or the LSTM model, please cite:
